...
def python_with_pip(explicit: str | None = None) -> str:
    """Choose a Python 3.12 builder, using uv when its environment omits pip."""

    candidates = [explicit, sys.executable, shutil.which("python")]
    checked: set[str] = set()
    uv_fallback: str | None = None
    for candidate in candidates:
        ...
        if probe.returncode == 0:
            return resolved
        if uv_fallback is None and shutil.which("uv"):
            uv_fallback = resolved
    if uv_fallback is not None:
        return uv_fallback
    raise RuntimeError("找不到带 pip 的 Python 3.12 wheel 构建器")


def _build_wheel(builder_python: str, output: Path) -> None:
    ...
    uv = shutil.which("uv")
    if not uv:
        raise RuntimeError("Python 3.12 不含 pip，且未找到 uv wheel 构建器")
    subprocess.run([
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _build_wheel(builder_python, output)
