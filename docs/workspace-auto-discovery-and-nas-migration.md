...
1. **约定优于配置**：默认以 `Agent-loop-system` 的同级目录 `../Agent-loop-workspace/{PROJECT}` 作为隔离源码工作区。
...
```text
/your-storage-or-nas/
├── Agent-loop-system/          # 本自动化测试与 Agent 系统仓
└── Agent-loop-workspace/       # 隔离固件工作区父目录
...
支持在 `.env` 中使用单一相对路径变量：
```ini
W30_HARDWARE_WORKSPACE=../Agent-loop-workspace/6202_W5230
W30_HARDWARE_PROJECT=6202_W5230
W30_HARDWARE_TRANSPORT=supercom
W30_HARDWARE_PORT=COM7
```
...
- **原因**：未显式配置路径，且在 `../Agent-loop-workspace/{PROJECT}` 或 `./workspaces/{PROJECT}` 中未找到目标项目。
- **解决**：
...
- **解决**：系统严禁将上游只读参考库作为真机工作区，请迁移或克隆至独立的隔离工作区（如 `Agent-loop-workspace/6202_W5230`）中执行。
