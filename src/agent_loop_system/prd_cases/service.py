...
P0_REVIEW_DIMENSIONS = ("wording", "classification", "order", "feature_boundary")
EXCEL_HEADERS = [
...
class PrdCaseService:
    ...
    def snapshot_path(self, job_id: str) -> Path:
        return self._job_dir(job_id) / "review-snapshot.json"
    ...
    def run_job(self, job_id: str, *, feedback: str = "") -> None:
        try:
            ...
            snapshot_file = self.snapshot_path(job_id)
            _atomic_json(snapshot_file, snapshot)
            snapshot_sha = _sha256(snapshot_file)
            state.update({
    ...
    def regenerate(self, job_id: str, feedback: str = "") -> dict[str, Any]:
        ...
        if not instruction:
            ...
            rev_comment = str(review.get("comment") or "").strip()
            case_comments = review.get("case_comments") or []
            parts: list[str] = []
            if rev_comment:
                parts.append(f"整体审查意见: {rev_comment}")
            if case_comments:
                cparts: list[str] = []
                for item in case_comments:
                    cid = str(item.get("stable_id") or item.get("case_id") or "").strip()
                    cmsg = str(item.get("comment") or "").strip()
                    if cmsg:
                        prefix = f"[{cid}] " if cid else ""
                        cparts.append(f"{prefix}{cmsg}")
                if cparts:
                    parts.append("逐条审查意见:\n" + "\n".join(cparts))
            if parts:
                instruction = "\n\n".join(parts)
            elif state.get("error"):
                instruction = f"修复生成错误: {state['error']}"
            elif state.get("quality_gate") and not state["quality_gate"].get("passed"):
                err_items = state["quality_gate"].get("errors", [])
                err_msgs = [f"- {e.get('code')}: {e.get('message')}" for e in err_items if isinstance(e, dict)]
                instruction = "修复质量门禁问题:\n" + "\n".join(err_msgs)
...
同一业务的正常、异常、边界、状态迁移分别拆分。每个步骤和预期都写成独立数组项。"""
    ...
    def get_cases(self, job_id: str) -> dict[str, Any]:
        snapshot_path = self.snapshot_path(job_id)
        ...
        return {
            ...
            "snapshot_sha256": snapshot.get("snapshot_sha256") or _sha256(snapshot_path),
    ...
    def _normalize_dimensions(self, dimensions: Any) -> dict[str, str]:
        if isinstance(dimensions, (list, set, tuple)):
            present = set(dimensions)
            if not all(d in present for d in P0_REVIEW_DIMENSIONS):
                missing = [d for d in P0_REVIEW_DIMENSIONS if d not in present]
                raise ValueError(f"PRD_P0_REVIEW_REQUIRED: 审批必须显式完成全部 4 项 P0 审查维度，缺少: {', '.join(missing)}")
            return {d: "PASS" for d in P0_REVIEW_DIMENSIONS}
        if isinstance(dimensions, dict):
            for d in P0_REVIEW_DIMENSIONS:
                val = dimensions.get(d)
                if val is not True and val != "PASS":
                    raise ValueError(f"PRD_P0_REVIEW_REQUIRED: P0 审查维度 '{d}' 未完成或未通过")
            return {d: "PASS" for d in P0_REVIEW_DIMENSIONS}
        raise ValueError("PRD_P0_REVIEW_REQUIRED: 审批必须显式完成 wording, classification, order, feature_boundary 全部 4 项 P0 审查维度")

    @staticmethod
    def _normalize_case_comments(
        comments: Any,
        ...
        if not isinstance(comments, list):
            raise ValueError("PRD_CASE_COMMENTS_INVALID")
        valid_ids = {
            str(case.get("stable_id") or case.get("case_id") or "").strip()
        ...
        for item in comments:
            if not isinstance(item, dict):
                raise ValueError("PRD_CASE_COMMENT_INVALID")
            stable_id = str(item.get("stable_id") or item.get("case_id") or "").strip()
            comment = str(item.get("comment") or "").strip()
            if not stable_id or stable_id not in valid_ids or not comment or stable_id in seen:
                raise ValueError("PRD_CASE_COMMENT_INVALID")
            normalized.append({"stable_id": stable_id, "comment": comment})
            seen.add(stable_id)
        return normalized
    ...
    def review(
        ...
        *,
        ...
        dimensions: dict[str, Any] | list[str] | None = None,
    ) -> dict[str, Any]:
        with self._lock:
            state = self._read_state(job_id)
            ...
            if not snapshot_file.is_file():
                raise ValueError("PRD_SNAPSHOT_MISSING: 审查快照不存在")
            snapshot_sha = _sha256(snapshot_file)
            if snapshot_sha != state.get("snapshot_sha256"):
                raise ValueError("PRD_SNAPSHOT_CHANGED: 审查快照已被修改，请重新生成")
            try:
                snapshot_data = json.loads(snapshot_file.read_text(encoding="utf-8"))
            except (OSError, UnicodeError, json.JSONDecodeError) as exc:
                raise ValueError("PRD_SNAPSHOT_CHANGED: 快照格式损坏") from exc
            if snapshot_data.get("workbook_sha256") != workbook_sha:
                raise ValueError("PRD_SNAPSHOT_WORKBOOK_MISMATCH: 审查快照与 Excel 不一致")

            comments = self._normalize_case_comments(
                case_comments,
                snapshot_data.get("cases") if isinstance(snapshot_data.get("cases"), list) else [],
            )
            if action == "reject":
                if not str(comment or "").strip() and not comments:
                ...
                norm_dims = self._normalize_dimensions(dimensions)
                review = {
                    "decision": "APPROVED",
                    "reviewer": reviewer,
                    "comment": str(comment or "").strip(),
    ...
    def sync(self, job_id: str) -> dict[str, Any]:
        with self._lock:
            ...
            if review.get("decision") != "APPROVED" or release.get("decision") != "GO":
                raise ValueError("PRD_SYNC_REQUIRES_APPROVAL")

            # Verify P0 dimensions
            review_dims = review.get("dimensions") or {}
            if isinstance(review_dims, list):
                review_dims = {k: True for k in review_dims}
            if not all(review_dims.get(k) for k in P0_REVIEW_DIMENSIONS):
                raise ValueError("PRD_P0_REVIEW_REQUIRED: 缺少 P0 审查维度，禁止同步")

            # Verify workbook hash binding
            workbook_sha = _sha256(self.workbook_path(job_id))
            if (
                workbook_sha != state.get("workbook_sha256")
                or workbook_sha != review.get("workbook_sha256")
                or workbook_sha != release.get("workbook_sha256")
            ):
                raise ValueError("PRD_WORKBOOK_CHANGED: Excel 文件与审批基线不一致")

            # Verify snapshot hash binding (Fail closed on snapshot tampering!)
            snapshot_file = self.snapshot_path(job_id)
            if not snapshot_file.is_file():
                raise ValueError("PRD_SNAPSHOT_MISSING: 审查快照不存在")
            snapshot_sha = _sha256(snapshot_file)
            if (
                snapshot_sha != state.get("snapshot_sha256")
                or snapshot_sha != review.get("snapshot_sha256")
                or snapshot_sha != release.get("snapshot_sha256")
            ):
                raise ValueError("PRD_SNAPSHOT_CHANGED: 审查快照与审批基线不一致，禁止同步")

            snapshot = json.loads(snapshot_file.read_text(encoding="utf-8"))
            if snapshot.get("workbook_sha256") != workbook_sha:
                raise ValueError("PRD_SNAPSHOT_WORKBOOK_MISMATCH")
            if not snapshot.get("gate", {}).get("passed"):
                raise ValueError("PRD_QUALITY_GATE_FAILED")

            project = self.project_lookup(state["project_id"])
            raw_cases = snapshot.get("cases") or snapshot.get("items") or []
            cases = [self._to_managed_case(case, project, state) for case in raw_cases]
