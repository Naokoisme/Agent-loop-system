...
def run_hardware_preflight(
    *,
    ...
    port = str(settings.get("W30_HARDWARE_PORT", "")).strip().upper()
    if not port:
        checks.append(
            _check(
                "supercom_pipe",
        ...
        return _finish(checks, project=project, checked_at=checked_at)
    try:
        ports = (serial_ports_probe or get_serial_ports_status)(port)
        ...
        configured = next(
            (item for item in items if str(item.get("port") or "").upper() == port),
            None,
        )
        if not configured or not configured.get("present") or not configured.get("supercom_open"):
            raise RuntimeError(f"{port} 对应的 SuperCom 管道不存在或不可用")
        checks.append(
            _check(
                "supercom_pipe",
                "pass",
                detail=f"{port} 对应的 SuperCom 管道已存在",
                diagnostics={
                    "configured_port": port,
                    "active_ports": active_ports,
                    "active_count": len(active_ports),
                    "pipe_path": str(configured.get("pipe_path") or ""),
                },
            )
        )
    except Exception as exc:
        checks.append(
            _check(
                ...
                detail=str(exc),
                diagnostics={"configured_port": port},
            )
        )
        return _finish(checks, project=project, checked_at=checked_at)
    ...
    try:
        ...
        checks.append(
            _check(
                "mtp_namespace",
                "pass",
                detail="ZORA → storage → download 命名空间可浏览",
                diagnostics={
                    "device": str(namespace.get("device") or "")[:100],
                    "storage": str(namespace.get("storage") or "")[:100],
                    "folder": str(namespace.get("folder") or "")[:100],
                },
            )
        )
    except Exception as exc:
